...
enum ToTokenizerMsg {
    // From HtmlTokenizer
    TokenizerResultDone,
    TokenizerResultScript(ParseNode),
...
impl Tokenizer {
    ...
    pub fn feed(&mut self, input: &mut BufferQueue) -> Result<(), Root<HTMLScriptElement>> {
        ...
        // The input contains VecDeque<StrTendril>, which is converted to
        // Vec<SendTendril>, so that it can be shared between threads with the use of a mutex.
        // I used a mutex here because we need to access `send_tendrils` again when we receive the
        // result of the feed function from the HtmlTokenizer. We use it to update the `input` parameter.
        let send_tendrils = Arc::new(Mutex::new(send_tendrils));

        // Send message to parser thread, asking it to start reading from the shared input.
        ...
        loop {
            match self.receiver.recv().expect("Unexpected channel panic in main thread.") {
                ...
                ToTokenizerMsg::TokenizerResultDone => return Ok(()),
    ...
    loop {
        match receiver.recv().expect("Unexpected channel panic in Html parser thread") {
            ToHtmlTokenizerMsg::Feed(data) => {
                ...
                let res = match html_tokenizer.feed(&mut buffer_queue) {
                    TokenizerResult::Done => ToTokenizerMsg::TokenizerResultDone,
                    TokenizerResult::Script(script) => ToTokenizerMsg::TokenizerResultScript(script)
                };
